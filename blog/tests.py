...
class ArticleTest(TestCase):
    ...
    def test_validate_article(self):
        ...
        self.__check_pagination__(p, 'Tags Archive', tag.slug)

        p = Paginator(
            Article.objects.filter(
                author__username='liangliangyy'), 2)
        self.__check_pagination__(p, 'Author Article Archive', 'liangliangyy')

        p = Paginator(Article.objects.filter(category=category), 2)
        self.__check_pagination__(p, 'Category Archives', category.slug)
